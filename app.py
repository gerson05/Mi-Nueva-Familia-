...
if uploaded_file is not None and selected_sponsor:
    ...
    if st.session_state.ai_results:
        ...
        with col_ex1:
            metodo_excel = st.selectbox("Método:", ["TRANSFERENCIA", "CONSIGNACION"])
        with col_ex2:
            # Buscar si el campo es la cedula por el ejemplo proporcionado
            cuenta_excel = st.text_input("Cuenta / Referencia Excel:", value=str(sponsor_data.get('Cédula / NIT', '')).replace(".0", ""))
        with col_ex3:
            lugar_excel = st.text_input("Lugar (Entidad):", value="BANCOLOMBIA")
